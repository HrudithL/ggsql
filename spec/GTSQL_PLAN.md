...
- `AS` is a SQL alias — renames the underlying column. Display labels are set via `LABEL <col> => '<text>'`.
...
**Example 4: Date formatting with column header label**
```sql
-- Render dates in long form; LABEL sets the displayed column header text
FORMAT order_date
  RENAMING * => '{:time %B %d, %Y}'
